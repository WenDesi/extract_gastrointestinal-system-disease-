...
def add_susceptibles(ontology,susceptibles_dict):
    new_ontology = []
    flag = False

    department_line = '		<oboInOwl:susceptibles rdf:datatype="&xsd;string">%s</oboInOwl:susceptibles>'
    ...
    for line in ontology:
        ...
        if len(line_disease) > 0:
            ...
            if disease in susceptibles_dict:
                flag =  True
                disease_susceptibles = susceptibles_dict[disease]
            else:
                flag = False

        elif line_end and flag:
            new_ontology.append(department_line % disease_susceptibles)

        new_ontology.append(line)

    return new_ontology

def build_susceptibles_dict(lines):
    dict = {}

    for line in lines:
        department = line[1:]
        doid = line[0].decode()
        dict[doid] = department[0]

    return dict


if __name__ == '__main__':
    full_ontology_filepath = u'zh_gastrointestinal_step2.owl'
    new_ontology_filepath = u'zh_gastrointestinal_step3.owl'

    susceptibles_filepath = 'susceptibles.csv'



    ontology = read_ontology(full_ontology_filepath)
    print 'end read'
    susceptibles_dict = build_susceptibles_dict(read_csv(susceptibles_filepath))

    print 'end read'

    new_ontology = add_susceptibles(ontology,susceptibles_dict)
